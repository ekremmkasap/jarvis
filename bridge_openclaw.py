...
# ─── OPENCLAW AGENT ──────────────────────────────────────────────────────────
try:
    from openclaw_skill import run_agent_task, run_shell_full, get_jarvis_status
    OPENCLAW_ENABLED = True
    log.info("OpenClaw Agent aktif")
except Exception as _oe:
    OPENCLAW_ENABLED = False
    log.warning(f"OpenClaw yuklenemedi: {_oe}")
    def run_agent_task(task, send_progress=None): return "OpenClaw aktif degil."
    def run_shell_full(cmd): return run_command_safe(cmd) if 'run_command_safe' in dir() else "Komut calistirilamadi."
    def get_jarvis_status(): return "Durum alinamadi."
# ─────────────────────────────────────────────────────────────────────────────
...
*Komutlar:*
  ...
  `/arama [sorgu]` -> Web arama (DuckDuckGo)
  `/scrape [url]` -> URL icerigini cek
  `/code [gorev]` -> Kod yaz
  `/plan [proje]` -> Plan olustur
  `/agent [gorev]` -> OpenClaw (Claude Code Pro)
  `/task [gorev]` -> OpenClaw alias
  `/durum` -> Tum servislerin ozet durumu
  `$ [komut]` -> Sunucu komutu (tam yetkili)
  `!! [komut]` -> sudo komutlari icin
...
4. Dropshipping fizibilitesi"""
    ...
    elif command in ("/arama", "/search"):
        query = args or "Jarvis"
        sys.path.insert(0, "/opt/jarvis/skills")
        try:
    ...
    elif command in ("/scrape", "/oku"):
        url = args or ""
        if not url:
            return "Kullanim: `/scrape https://example.com`"
        sys.path.insert(0, "/opt/jarvis/skills")
        try:
            from web_search_skill import scrape_url
            result = scrape_url(url, max_chars=2000)
            return f"*{url}*\n\n{result}"
        except Exception as e:
            return f"Scrape hatasi: {e}"
    ...
    elif command in ("/agent", "/task", "/gorev-agent"):
        if not args:
            return (
                "*OpenClaw Agent*\n\n"
                "Karmasik gorevler icin Claude Code Pro kullanir.\n\n"
                "Kullanim:\n"
                "`/agent bridge.py'ye yeni komut ekle`\n"
                "`/task sunucudaki servisleri kontrol et`\n"
                "`/agent ollama modelleri listele ve en iyisini onerir`"
            )
        return run_agent_task(args)
...
def process_message(chat_id: int, text: str) -> str:
    ...
    if text.startswith("$ "):
        ...
        return f"```\n{result[:3500]}\n```"
    if text.startswith("!! "):
        # sudo komutlari icin: !! echo userk1 | sudo -S systemctl restart jarvis
        cmd = text[3:].strip()
        result = run_shell_full(cmd)
        memory.add_message(chat_id, "user", text)
        memory.add_message(chat_id, "assistant", result, "system")
