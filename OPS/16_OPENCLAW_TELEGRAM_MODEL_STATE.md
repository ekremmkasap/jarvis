# OPS 16 - OpenClaw Telegram Model State

Durum: partial
5H adaptation note: yeni prompt model/auth/profile ayrimini daha sert istiyor; bu dosya o lanein birincil kaydi olmaya devam eder.
...
tarihsel olarak olmayan `server/openclaw/bridge.py` yoluna bakiyordu

Bu sprintte yapilan:
- `openclaw.cmd` canonical `server/bridge.py` wrapper'ina cevrildi
- `install_openclaw_startup.cmd` ayni mantiga cevrildi
- `openclaw_web_only.cmd` audit edildi ve `server/bridge.py --web-only` delegasyonu dogrulandi
