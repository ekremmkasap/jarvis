...
def _resolve_octogent_command() -> str:
    return _resolve_global_octogent_command()


def _repo_wrapper_command() -> str:
    candidate = ROOT_DIR / Path(OCTOGENT_COMMAND).name
    return str(candidate) if candidate.is_file() else ""


def _resolve_global_octogent_command() -> str:
    command_name = Path(OCTOGENT_COMMAND).name or ("octogent.cmd" if os.name == "nt" else "octogent")
    ...
    if os.name == "nt":
        ...
        if appdata:
            candidates.append(appdata / "npm" / "octogent.cmd")
    else:
        candidates.append(Path.home() / ".npm-global" / "bin" / "octogent")
    ...
    repo_root = str(ROOT_DIR.resolve()).lower()
    for candidate in candidates:
        ...
        if not resolved_path.is_file():
            continue
        if str(resolved_path).lower().startswith(repo_root):
            continue
...
def build_octogent_health_snapshot() -> dict[str, Any]:
    ...
    repo_wrapper_command = _repo_wrapper_command()
    command_source = "global" if resolved_command else "repo-wrapper" if repo_wrapper_command else "missing"
    ...
    capabilities = {
        ...
        "cli_ready": {
            "ok": command_source == "global",
            "detail": "command-ready" if command_source == "global" else "command-missing",
        },
        "project_scaffold": {
    ...
    return {
        **descriptor,
        ...
        "repo_wrapper_command": repo_wrapper_command,
