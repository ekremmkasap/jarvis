...
# The bash script that will be executed on the Kali server to build the architecture
...
try:
    print(f"Connecting to {host} to build architecture...")
    client.connect(hostname=host, port=port, username=username, password=password, timeout=10)
    
    # create script via SFTP
    sftp = client.open_sftp()
    with sftp.file('/home/userk/setup_jarvis.sh', 'w') as f:
        f.write(setup_script)
    sftp.close()
    
    # execute
    cmd = "chmod +x /home/userk/setup_jarvis.sh && /home/userk/setup_jarvis.sh"
